...
class GitRepositoryManager:
            ...
            with sqlite3.connect(self.db_path) as conn:
                conn.execute("""
                    CREATE TABLE IF NOT EXISTS git_repositories (
                        ...
                        category TEXT NOT NULL CHECK (category IN ('configs', 'templates')),
                        url TEXT NOT NULL,
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.execute("""
                    ...
                    (name, category, url, branch, username, token, path, verify_ssl, description, is_active)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    repo_data["name"],
                    repo_data["category"],
    ...
    def update_repository(self, repo_id: int, repo_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            for field in ["name", "category", "url", "branch", "username", "token", "path", "verify_ssl", "description", "is_active"]:
