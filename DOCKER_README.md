...
This document describes the container setup that runs both the frontend and backend in a single container for improved convenience and resource efficiency.
...
# Manual health check
docker exec cockpit curl -f http://localhost:3000/
docker exec cockpit curl -f http://localhost:8000/docs
```
...
- `docker-compose.yml` - Updated for container
